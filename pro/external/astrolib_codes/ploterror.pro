PRO ploterror, x, y, xerr, yerr, NOHAT=hat, HATLENGTH=hln, ERRTHICK=eth, $
      ...
      NSKIP = nskip, NOCLIP = noclip, ecolOR = ecol, YRANGE = yrange, $
...
;                  ERRTHICK=, ERRSTYLE=, ecolOR=, NSKIP=, .. PLOT keywords]
;
; INPUTS:
;     X = array of abcissae.
;     Y = array of Y values.
...
;     ecolOR =  scalar integer (0 - !D.N_TABLE) or string (e.g. 'red')
...
;            
;
;     Any valid keywords to the cgPLOT command (e.g. PSYM, YRANGE, AXISCOLOR  ;     SYMCOLOR, ASPECt) are also accepted by PLOTERROR via the _EXTRA facility.
;
; RESTRICTIONS:

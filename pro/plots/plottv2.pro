...
pro plottv2, z, x, y, scale = scale, xrange=xrange, yrange = yrange, zrange = zrange, $
            ...
            minpeaks=minpeaks, polvec=polvec, colvec=colvec
...
; color scale
x0     = position[2]+dx/15. & dxscl = dx/20.
posscl = [x0,position[1],x0+dxscl,position[3]]
zmin   = min(zz,max=zmax)
if (n_elements(zrange) eq 2) then begin ;SL
   zmin = zrange[0]                     ;SL
   zmax = zrange[1]                     ;SL
endif                                   ;SL
plot,/nodata,[0,1],[zmin,zmax], position=posscl, xticks=1,yticks=1, $
     xtickn=empty,ytitle=zunits, xstyle=4, ystyle=4, charsize=charsize,/noerase,xthick=3,ythick=3   
...
;    tvscl, ramp2, posscl[0]+0.01*dxscl, posscl[1], xsize = dxscl*0.98, ysize = dy,/NORMAL
endif else begin
   tv,    ramp2, posscl[0], posscl[1], xsize = dxscl, ysize = dy,/NORMAL
endelse
axis,yaxis=1,ytitle=zunits,charsize=charsize,yrange=[zmin,zmax],ystyle=1,yticklen=0.4

if do_contours then begin
    contour, ramp2, levels=contours,/noerase, position=posscl, xticks=1,$
             xtickn=empty, ytickn=empty, xstyle=5, ystyle=5, c_linestyle = c_linestyle
endif
; -------------------
...
;Plot the map
if do_scale then begin
   imdisp, map, position=position,xsize = dx, ysize = dy,/NORMAL,range=[zmin,zmax] ;SL
...
if do_contours then begin
    contour,map,levels=contours,/noerase,xtickname=empty,$
            ytickname=empty,xticks=1,yticks=1, xstyle=style,$
            ystyle=style,pos=position, c_linestyle=c_linestyle
endif
...
if (keyword_set(nolabels) and not keyword_set(noaxis)) then begin
    plot,[p[0],p[2],p[2],p[0],p[0]],[p[1],p[1],p[3],p[3],p[1]],_extra = extrakw,$
         xstyle=style, ystyle=style, pos= position,/normal,/noerase
endif
...
;   yy2 = res[1,*] - 0.5*polvec[0,*]*cos(polvec[3,*]*!dtor)
   ...
   arrow, xx1, yy1, xx2, yy2, hsize=0.01, col=colvec, thick=3.8
Endif

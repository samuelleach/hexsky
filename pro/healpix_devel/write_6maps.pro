...
;
; -----------------------------------------------------------------------------
pro write_6maps, fitsfile, TQU, Coordsys=coordsys, Nested=nested, Ring=ring, Ordering=ordering, Extension=extension_id, Hdr=hdr, Xhdr=xhdr, Units=units, Help=help
;+
; NAME:
;    write_6maps  - Based on write_tqu
;
; PURPOSE:
...
if (nmap ne 1 && nmap ne 3 &&  nmap ne 4 && nmap ne 6 && nmap ne 5) then message,' Expect a Npix map or Npix * [3 or 4]  TQU array' ;SML
...
if (madam_format) then begin
; name columns
    name =  ['cc_1_1','cc_1_2','cc_1_3','cc_2_2','cc_2_3','cc_3_3']
; extension names
    xtname = ['MADAM style N_inv']
endif

if (my_format) then begin
; name columns
    name =  ['TEMPERATURE',  'Q_POLARISATION',  'U_POLARISATION','dI1','dI2']
; extension names
    xtname = ['SIGNAL']
endif
...
;    if (nmap eq 1) then begin
    case nmap of
        1: xtns = create_struct('HDR', xhdr, $
                                name[0,i_ext], TQU[*,0,ia])
        3: xtns = create_struct('HDR', xhdr, $
                                name[0,i_ext], TQU[*,0,ia],$
                                name[1,i_ext], TQU[*,1,ia],$
                                name[2,i_ext], TQU[*,2,ia])
        5: xtns = create_struct('HDR', xhdr, $
                                name[0,i_ext], TQU[*,0,ia],$
                                name[1,i_ext], TQU[*,1,ia],$
                                name[2,i_ext], TQU[*,2,ia],$
                                name[3,i_ext], TQU[*,3,ia],$
                                name[4,i_ext], TQU[*,4,ia])
        6: xtns = create_struct('HDR', xhdr, $
                                name[0,i_ext], TQU[*,0,ia],$
                                name[1,i_ext], TQU[*,1,ia],$
                                name[2,i_ext], TQU[*,2,ia],$
                                name[3,i_ext], TQU[*,3,ia],$
                                name[4,i_ext], TQU[*,4,ia],$
                                name[5,i_ext], TQU[*,5,ia])
	else: print,'nmap = ',nmap
    endcase
    if (wmap_format) then begin
